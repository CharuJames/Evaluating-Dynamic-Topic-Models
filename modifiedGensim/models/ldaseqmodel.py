...
class LdaSeqModel(utils.SaveLoad):
    ...
    def fit_lda_seq(self, corpus, lda_inference_max_iter, em_min_iter, em_max_iter, chunksize):
       ...
       """
        ...
        while iter_ < em_min_iter or ((convergence > LDASQE_EM_THRESHOLD) and iter_ <= em_max_iter):
            ...
            b.append(math.exp(bound))
            self.gammas = gammas
            print("bound :",bound, math.exp(bound))
    ...
    def plot(self,topic1_loss):
        df = pd.DataFrame()
        df['loss'] = (topic1_loss)
        df.to_csv('/scratch/james/large_files/AllfromHome/EMNLP/DLDA/loss_plot25.csv')
        print(topic1_loss, len(topic1_loss))
        fig = plt.figure()
        epoch = range(len(topic1_loss))
        plt.plot(epoch, topic1_loss, 'g', label='Training loss')
        plt.title('Training loss')
        plt.xlabel('em iterations')
        plt.ylabel('Loss')
        plt.legend()
        fig.savefig('/scratch/james/large_files/AllfromHome/EMNLP/DLDA/loss_plot125.png')
